...
async function getRecordByid(sn) {	  
	try { 
		if(sn){ 
			var sql='select * from byd_careers where sn = '+ sn;   
...
async function deleteRecord(sn) {	  
	try { 
		if(sn){ 
			var sql='delete  from byd_careers where sn = '+ sn;   
			return new Promise((resolve,reject)=>{
				connectPool.query(sql, (err, result) => {
					if (err) { 
						reject(err)
...
async function deactivateRecord(sn) {	  
	try { 
		if(sn){ 
			var sql='update byd_careers set status=0 where sn = '+ sn;   
			return new Promise((resolve,reject)=>{
				connectPool.query(sql, (err, result) => {
					if (err) { 
						reject(err)
...
async function activateRecord(sn) {	  
	try { 
		if(sn){ 
			var sql='update byd_careers set status=1 where sn = '+ sn;   
			return new Promise((resolve,reject)=>{
				connectPool.query(sql, (err, result) => {
					if (err) { 
						reject(err)
...
async function getAllData() {	  
	try {   
			var sql='select * from byd_careers where 1=1 ORDER BY sn DESC';    
			return new Promise((resolve,reject)=>{
				connectPool.query(sql, (err, result) => {
					if (err) { 
						reject(err)
...
async function saveData(data) { 
	try { 
		if(data){   
			var sql='INSERT INTO byd_careers set ? ';
			return new Promise((resolve,reject)=>{
...
async function updateData(data) {	  
	...
	try { 
		if(data){   
			var sql = "UPDATE byd_careers set ? WHERE sn = ?";   
			return new Promise((resolve,reject)=>{
...
module.exports={
	...
	{	
        var Product = sequelize.define('careers', {
              sn: { 
                type: type.INTEGER,
                primaryKey: true,
                autoIncrement: true
