...
async function getRecordByid(sn) {	  
	try { 
		if(sn){ 
			var sql='select * from newsletter where sn = '+ sn;   
...
async function deleteRecord(sn) {	  
	try { 
		if(sn){ 
			var sql='delete  from newsletter where sn = '+ sn;   
			return new Promise((resolve,reject)=>{
				connectPool.query(sql, (err, result) => {
					if (err) { 
						reject(err)
...
async function getAllData() {	  
	try {   
			var sql='select * from newsletter where 1 = 1 ORDER BY sn DESC';    
			return new Promise((resolve,reject)=>{
				connectPool.query(sql, (err, result) => {
					if (err) { 
						reject(err)
...
async function updateData(data) {	  
	...
	try { 
		if(data){   
			var sql = "UPDATE newsletter set ? WHERE sn = ?";   
			return new Promise((resolve,reject)=>{
