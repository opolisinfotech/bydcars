 ...
 *  Returns: void 
*/
async function getByProductId(dealers_id) {	  
	try {   
			var sql='select * from '+table+' where dealers_id ='+dealers_id;
			console.log(sql);    
			return new Promise((resolve,reject)=>{
				connectPool.query(sql, (err, result) => {
					if (err) { 
 ...
 *  Returns: void 
*/
async function setDefaultImage(id, dealers_id) {	  
	try { 
		if(id){ 

			var sql1='UPDATE '+table+' set default_image = 0 where dealers_id = '+ dealers_id;  
 ...
 *  Returns: void 
*/
async function getDefaultImage(dealers_id) {	  
	try {   
		var sql='select * from '+table+' where dealers_id ='+dealers_id+' AND  default_image = 1'; 
			return new Promise((resolve,reject)=>{
				connectPool.query(sql, (err, result) => {
					if (err) { 
						reject(err)
...
module.exports={
	...
	{	
        var ProductImages = sequelize.define(table, {
              id: {
                type: type.INTEGER,
                primaryKey: true,
                autoIncrement: true
              },  
              dealers_id : { type: type.INTEGER }, 
