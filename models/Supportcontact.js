...
async function getRecordByid(sn) {	  
	try { 
		if(sn){ 
			var sql='select * from support_contact where sn = '+ sn;   
...
async function deleteRecord(sn) {	  
	try { 
		if(sn){ 
			var sql='delete  from support_contact where sn = '+ sn;   
			return new Promise((resolve,reject)=>{
				connectPool.query(sql, (err, result) => {
					if (err) { 
						reject(err)
...
async function getAllData() {	  
	try {   
			var sql='select * from support_contact where 1 = 1 ORDER BY sn DESC';    
			return new Promise((resolve,reject)=>{
				connectPool.query(sql, (err, result) => {
					if (err) { 
						reject(err)
...
async function saveData(data) { 
	try { 
		if(data){   
			var sql='INSERT INTO support_contact set ? ';
			return new Promise((resolve,reject)=>{
...
async function updateData(data) {	  
	...
	try { 
		if(data){   
			var sql = "UPDATE support_contact set ? WHERE sn = ?";   
			return new Promise((resolve,reject)=>{
